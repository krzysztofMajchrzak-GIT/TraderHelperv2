...
def funkcja_obciągająca(interval, window_size, endTime=None):
    ...
    #  !
    url = f"https://api.binance.com/api/v3/klines?symbol=ETHUSDT&interval={interval.replace('T', 'm')}&limit={window_size}"
    if endTime:
        endTime = endTime.tz_localize(timezone('Europe/Warsaw'))
    ...
    if ((endTime or datetime.now(timezone('UTC'))) - df['time'].iloc[-1]).seconds // 60 < int(interval[:-1]):
        return df.iloc[:-1]
    else:
        return df.iloc[1:]
...
def process(df, modifier, columns):
    COLUMN_SET = set(columns)
    window_size = 14

    # Apply the Savitzky-Golay filter to all columns
    # df[['open', 'high', 'low', 'close', 'Volume ETH']] = df[['open', 'high', 'low', 'close', 'Volume ETH']].apply(lambda col: savgol_filter(col, 10, 2))
    ...
    if 'ROC' in COLUMN_SET:
        # Calculate the 9-period Rate of Change (ROC) using Close prices
